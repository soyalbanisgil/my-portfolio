import React from "react";
import './Skills.styles.sass';

const Skills = () => {
  return (
    <section id="skills" className="pt-4" data-aos="zoom-in">
      <div className="container text-center py-5 my-5">
        <h4>My current <br/><span>stack of languages and technologies</span></h4>
        <p className="py-4">
          HTML5 - CSS3 - JAVASCRIPT - SASS - REACT.JS - FIREBASE - BABEL -
          BOOTSTRAP - JQUERY - PHP - MYSQL - POSTGRESQL - NODE.JS - EXPRESS - NPM - YARN - 
          GIT - GITHUB
        </p>
        <a href="https://drive.google.com/open?id=1ux3x8EyUhwh0F4K3K0D2yzM4-vRDvWOf" target="_blank" rel='noopener noreferrer' className="btn custom-btn">My Resume</a>
      </div>
    </section>
  );
};

export default Skills;
